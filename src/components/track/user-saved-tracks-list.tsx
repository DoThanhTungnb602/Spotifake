...
import { ISavedTrack } from "@/type";

import { isSavedTracks } from "@/lib/track";

export function SavedTracksList({ tracks }: { tracks: ISavedTrack[] }) {
  const [user] = useLocalStorage<string>("user");

  const checkSavedTracksQuery = useQuery({
    queryKey: ["check-saved-tracks", "saved-tracks"],
    ...
    <table className="w-full">
      <thead>
        <tr className="top-16 h-9 border-b-[1px] text-text-blur">
          ...
          <th className="text-start text-sm font-light">Date added</th>
